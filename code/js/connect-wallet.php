		...
		async function connectWallet(method) {
			...
			if (method == 1){
				...
				if (account){
					...
					//Make sure they got it loaded in a few seconds or offer them preview version
					setTimeout(offerNoAccountPreview(1), 7000);
				}
			}
			//User is taking a test
			else if (method == 2){
				if (account){
					...
					setTimeout(offerNoAccountPreview(2), 7000);
				}
			}
			//User is getting his or her tests
			else if (method == 3){
				if (account){
					//userAccountNumber.innerHTML = "User Account: " + account;
					window['userAccountNumber'] = account;
			...
			//User is getting his or her test results
			else if (method == 4){
				if (account){
					//Loading wheel;
					document.getElementById('connection-in-progress').style.display = "block";	
					userAccountNumber.innerHTML = "User Account: " + "<a href='/my-tests'>" + account + "</a>";
					window['userAccountNumber'] = account;
					//Create Web3 Object
